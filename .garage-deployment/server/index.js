globalThis.__VINEXT_LAZY_CHUNKS__ = ["garage/assets/layout-segment-context-DeST8Q4P.js","garage/assets/garage-app-DYSfgCze.js"];
...
function getDraftSecret() {
	return "8c322eb6-6ad2-443f-a538-96e023591b56";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "d3607cd4-bd90-4d9c-957d-b27cd6c6bb4b";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "d3607cd4-bd90-4d9c-957d-b27cd6c6bb4b") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "d3607cd4-bd90-4d9c-957d-b27cd6c6bb4b",
		rootBoundaryId
	});
}
/**
...
var CREATE_RECIPES_TABLE = `CREATE TABLE IF NOT EXISTS custom_recipes (
  id TEXT PRIMARY KEY NOT NULL,
  manufacturer TEXT NOT NULL,
  film_name TEXT NOT NULL,
  format_json TEXT NOT NULL,
  exposure_label TEXT NOT NULL,
  exposure_index INTEGER NOT NULL CHECK(exposure_index > 0),
...
var CREATE_FILM_INDEX = "CREATE INDEX IF NOT EXISTS idx_custom_recipes_film_name ON custom_recipes(film_name)";
...
function recipeFromRow(row) {
	...
	return {
		...
		version: `shared-${row.created_at.slice(0, 10)}-${row.id.slice(-6)}`
	};
}
function parseRecipe(value) {
	...
	if (!manufacturer || !filmName || !exposureLabels.has(exposureLabel) || !Number.isInteger(exposureIndex) || exposureIndex < 1 || exposureIndex > 1e5 || !Number.isInteger(baseSeconds) || baseSeconds < 1 || baseSeconds > 14400) return null;
	return {
		manufacturer,
		filmName,
...
async function ensureSchema(db) {
	schemaReady ??= db.batch([
		db.prepare(CREATE_RECIPES_TABLE),
		db.prepare(CREATE_FILM_INDEX),
		db.prepare("PRAGMA optimize")
	]).then(() => void 0);
	await schemaReady;
}
async function handleCustomRecipes(request, db) {
	await ensureSchema(db);
	if (request.method === "GET") {
		const result = await db.prepare("SELECT id, manufacturer, film_name, format_json, exposure_label, exposure_index, base_seconds, source_url, source_notes, created_at FROM custom_recipes ORDER BY film_name COLLATE NOCASE, created_at DESC").all();
	...
	await db.prepare("INSERT INTO custom_recipes (id, manufacturer, film_name, format_json, exposure_label, exposure_index, base_seconds, source_url, source_notes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, recipe.manufacturer, recipe.filmName, JSON.stringify(recipe.format), recipe.exposureLabel, recipe.exposureIndex, recipe.baseSeconds, recipe.sourceUrl, recipe.sourceNotes, now, now).run();
...
var worker_entry_default = { async fetch(request, env, ctx) {
	const url = new URL(request.url);
	if (url.pathname === "/garage/api/custom-recipes") return handleCustomRecipes(request, env.DB);
